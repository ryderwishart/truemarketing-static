...
jQuery(document).ready(function () {
	...
	jQuery( this )
		.on('click', '#thrive_meta_social_button_upload', ThrivePostOptions.ThriveSocialImageUploader)
		.on( 'change', 'select[id^="post-format-selector-"]', ThrivePostOptions.display_post_format_options )
		.on( 'click', '.post-format', ThrivePostOptions.display_post_format_options );
...
ThrivePostOptions.display_post_format_options = function () {
	var $wp5_format_select = jQuery('select[id^="post-format"]'),
		_selected_format = $wp5_format_select.length ? $wp5_format_select.val() : jQuery(".post-format:checked").val();
